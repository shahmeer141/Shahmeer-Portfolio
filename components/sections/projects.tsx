      ...
      <div className="grid gap-5 md:grid-cols-2">
        {projects.map((project) => {
          ...
          const card = (
            <div
              className={cn(
                "group h-full rounded-2xl border border-sage/15 bg-[#101714]/70 p-6 transition-colors hover:border-sage/40 hover:bg-moss/50 sm:p-8",
            ...
            >
              ...
              <div className={cn("mt-5 flex flex-wrap gap-2", project.featured && "lg:mt-0 lg:self-end")}>
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-sage/20 px-3 py-1 text-xs text-sage-bright"
                  >
                    {tag}
                  </span>
                ))}
          ...
          if (!project.href) {
            return (
              <div key={project.title} className={span}>
                {card}
              </div>
            );
          }
